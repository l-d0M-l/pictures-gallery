function renderData(data, page, totalPages) {
  ...
  data.map((element) => {
    ...
    savedDataLocalStorage.map((elementLocal) => {
      // console.log(element.id === elementLocal.id);

      // console.dir(element);
      // console.dir(elementLocal);
      if (elementLocal.id === element.id) {
        classToAdd += ` save-btn-saved`;
      }
    });
    ...
    <button class='${classToAdd}' title='Save to favorites' data-id='${element.id}'>
      <!-- Bookmark Icon SVG -->
...
function renderFavourites(data) {
  ...
  data.map((element) => {
    ...
    <button class='save-btn save-btn-saved' title='Save to favorites' data-id='${element.id}'>
      <!-- Bookmark Icon SVG -->
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        <path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path>
      </svg>
...
export { renderData, renderFavourites };
